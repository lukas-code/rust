...
fn control_flow() {
    if (return) {};
    let _ = |_| if (yield) {};
    let _: Result<(), ()> = try { if (do yeet) {} };

    if (return {}) {}; //~ ERROR unnecessary parentheses
    let _ = |_| if (yield {}) {}; //~ ERROR unnecessary parentheses
    let _: Result<(), ()> = try { if (do yeet {}) {} }; //~ ERROR unnecessary parentheses
}
...
fn struct_literal() {
    ...
    if (Foo::default() == Foo {}) {}
    if (Foo {} == Foo::default()) {}
    if (Foo::default() == Foo::default()) {} //~ ERROR unnecessary parentheses
