    ...
    for (event, event_range) in parser {
        match event {
            ...
            Event::End(_) => {
                ...
                if let Some(backtick_index) = element.backtick_index {
                    let (span, span_is_backtick) = match source_span_for_markdown_range(
                        tcx,
                        &dox,
                    ...
                    cx.tcx.struct_span_lint_hir(crate::lint::UNESCAPED_BACKTICKS, hir_id, span, "unescaped backtick", |lint| {
                        ...
                        match element.prev_code_guess {
                            ...
                            PrevCodeGuess::End { guess, .. } => {
                                // "`foo `bar`" -> "`foo` `bar`"
                                // Don't `clamp_end` here, because the suggestion is guaranteed
                                // to be inside an inline code node.
                        ...
                        if span_is_backtick {
                            lint.span_suggestion(span.shrink_to_lo(), "if you meant to use a literal backtick, escape it", "\\", Applicability::MaybeIncorrect);
                        } else {
            ...
            Event::Code(_) => {
                ...
                // This is a inline code might be longer than it's supposed to.
            ...
            Event::Text(text) => {
                ...
                // Don't lint html-escaped backticks (&#96;).
                if *text == *"`" && !is_escaped && *text == dox[event_range.clone()] {
                    // We found a stay backtick.
                    assert!(
                        element.backtick_index.is_none(),
                        "multiple unescaped backticks in the same element"
                    );
...
enum PrevCodeGuess {
    None,
    Start { guess: usize, is_confident: bool },
    End { guess: usize, is_confident: bool },
}
...
fn guess_start_of_code(dox: &str, range: Range<usize>) -> Option<usize> {
    ...
    if !can_suggest_backtick(dox, guess) {
        return None;
    }
...
fn suggest_insertion(
    ...
    {
        lint.span_suggestion(span, message, "`", Applicability::MaybeIncorrect);
    } else {
